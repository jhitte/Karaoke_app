...
class KaraokeSyncApp:
    ...
    def map_lyrics_to_timestamps(self, lyrics_path, audio_path, word_timestamps):
        ...
        for word in word_timestamps:
            if hasattr(word, 'start_time') and hasattr(word, 'word'):
                end_time = getattr(word, 'end_time', word.start_time + 0.5)
                if end_time - word.start_time >= 0.2:  # Filter short durations
                    timestamps.append({"start": word.start_time, "end": end_time, "word": word.word.lower()})
        
        logging.info(f"Detected {len(timestamps)} timestamps for {lyrics_path}")
        
        # Get audio duration with Librosa
        try:
            audio_duration, _ = librosa.get_duration(path=audio_path)
            logging.info(f"Audio duration for {audio_path}: {audio_duration}s")
        except Exception as e:
            logging.error(f"Failed to get duration for {audio_path}: {e}")
            audio_duration = 300.0  # Fallback
        
        # Map timestamps to lyrics lines
        num_timestamps = len(timestamps)
        first_line_time = 21.0
        
        if num_timestamps == 0:
            logging.warning(f"No valid timestamps for {lyrics_path}. Using interpolation.")
            ...
            # Split lyrics into words for alignment
            lyrics_words = []
            line_boundaries = []
            current_word_idx = 0
            for line in lines:
                ...
                line_boundaries.append((current_word_idx, current_word_idx + len(words)))
                current_word_idx += len(words)
            
            # Match timestamps to lyrics words
            matched_timestamps = []
            lyric_idx = 0
            timestamp_idx = 0
            
            while lyric_idx < len(lyrics_words) and timestamp_idx < len(timestamps):
                if lyrics_words[lyric_idx] == timestamps[timestamp_idx]["word"]:
                    matched_timestamps.append(timestamps[timestamp_idx])
                    lyric_idx += 1
                    timestamp_idx += 1
                elif lyric_idx < len(lyrics_words) - 1 and lyrics_words[lyric_idx + 1] == timestamps[timestamp_idx]["word"]:
                    lyric_idx += 1  # Skip mismatched word in lyrics
                else:
                    timestamp_idx += 1  # Skip mismatched timestamp
            
            logging.info(f"Matched {len(matched_timestamps)} words out of {len(lyrics_words)} lyrics words")
            
            # Map matched timestamps to lines
            mapped_timestamps = []
            for start_idx, end_idx in line_boundaries:
                line_timestamps = [t for t in matched_timestamps[start_idx:end_idx] if t]
                if line_timestamps:
                    mapped_timestamps.append({"start": line_timestamps[0]["start"], "end": line_timestamps[-1]["end"] if line_timestamps[-1]["end"] else line_timestamps[0]["start"] + 2.0})
                else:
                    mapped_timestamps.append({"start": first_line_time, "end": first_line_time + 2.0})
                    first_line_time += 2.0  # Increment for next missing line
            
            # If fewer timestamps than lines, interpolate remaining
            if len(mapped_timestamps) < num_lines:
                remaining_lines = num_lines - len(mapped_timestamps)
                last_time = mapped_timestamps[-1]["end"] if mapped_timestamps else first_line_time
                step = (audio_duration - last_time) / max(remaining_lines, 1)
                for i in range(remaining_lines):
                    start = last_time + i * step
                    end = start + min(step, 2.0)
                    mapped_timestamps.append({"start": start, "end": end})
            
            # Shift to start at ~21s
            if mapped_timestamps and mapped_timestamps[0]["start"] < first_line_time:
                offset = first_line_time - mapped_timestamps[0]["start"]
                for t in mapped_timestamps:
                    t["start"] += offset
                    t["end"] += offset
        
        logging.info(f"Mapped {len(mapped_timestamps)} timestamps to {num_lines} lyrics lines")
